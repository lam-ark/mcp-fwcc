...
## 🛠️ 5. Implementation Phases & Milestones
...
| **Phase 4** | **Slot-Base API References & KB** | Ingest 138 Slot-Base API docs + 15 Knowledge Base Q&As (`14_slot_base_docs/`, `15_knowledge_base/`). | ~150 Docs |
| **Phase 5** | **Search Engine & Tool Suite Upgrade** | Implement code tokenizer, field weighting, windowed chunking, and benchmark validation. | Code + Tools |

---

## ✅ 6. Benchmark Validation Matrix

Run automated test queries to verify search precision:

| Benchmark Query | Expected Top Hit | Validation Criterion |
| :--- | :--- | :--- |
| `2 blur scatter symbol appear in 1 reel when reel is spinning` | `BS_table_SlotReel_code.md` / `07_gotchas/02_duplicate_blur_scatter.md` | Top-1 hit contains `EXCEPTION_SYMBOLS` & `getRandomSymbol` fix. |
