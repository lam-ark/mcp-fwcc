---
id: "cc_core_lib:CustomCanvasSizeWrapper:recipes:usage"
title: "Recipe: Practical CustomCanvasSizeWrapper Integration in Slot Games"
category: "cc_core_lib"
tags: ["CustomCanvasSizeWrapper", "custom_canvas_size_wrapper", "cc_core_lib", "recipes", "integration"]
---

# 💡 Recipe: Practical `CustomCanvasSizeWrapper` Integration

---

## 1. Standard Initialization & Usage

```typescript
import { CustomCanvasSizeWrapper } from "cc-core-lib";

// 1. Accessing via global eno namespace or modular import
const instance = new CustomCanvasSizeWrapper();

// 2. Executing primary operations
// ...
```

---

## 2. Real-World Game Workflow Example (e.g. Red Cliff 9666)

In production slot titles like **Red Cliff (g9666)**, `CustomCanvasSizeWrapper` integrates directly into the Director and Writer modules to maintain seamless state synchronization across Base Game, Free Spins, and Respin cascades.
