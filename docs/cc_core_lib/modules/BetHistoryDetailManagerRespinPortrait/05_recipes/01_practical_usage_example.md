---
id: "cc_core_lib:BetHistoryDetailManagerRespinPortrait:recipes:usage"
title: "Recipe: Practical BetHistoryDetailManagerRespinPortrait Integration in Slot Games"
category: "cc_core_lib"
tags: ["BetHistoryDetailManagerRespinPortrait", "bet_history_detail_manager_respin_portrait", "cc_core_lib", "recipes", "integration"]
---

# 💡 Recipe: Practical `BetHistoryDetailManagerRespinPortrait` Integration

---

## 1. Standard Initialization & Usage

```typescript
import { BetHistoryDetailManagerRespinPortrait } from "cc-core-lib";

// 1. Accessing via global eno namespace or modular import
const instance = new BetHistoryDetailManagerRespinPortrait();

// 2. Executing primary operations
// ...
```

---

## 2. Real-World Game Workflow Example (e.g. Red Cliff 9666)

In production slot titles like **Red Cliff (g9666)**, `BetHistoryDetailManagerRespinPortrait` integrates directly into the Director and Writer modules to maintain seamless state synchronization across Base Game, Free Spins, and Respin cascades.
